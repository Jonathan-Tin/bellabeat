...
CREATE TABLE cs2.caloriestable (	
								id CHAR(10),
								activityday CHAR(10),
								calories INTEGER);
                    
CREATE TABLE cs2.dailyactivity (
							id CHAR(10),
...
/* Cleaning completed, analysis queries starts */ 
...
SELECT sub.id,
       ...
       ROUND(30 - (AVG(sub.active_minutes) OVER(PARTITION BY sub.id))/20) AS minutes_to_goal
   FROM (
   SELECT id,
...
-- We can also check where the users are ranked among total steps taken, total distance, and overall activeness
SELECT sub.*
FROM (
		SELECT 
			id,
			totalsteps,
			PERCENT_RANK() OVER (ORDER BY totalsteps DESC) AS percentile_among_peers
		FROM cs2.dailyactivity
		GROUP BY id
	 ) AS sub
WHERE sub.id IN('4319703577', '4558609924','6962181067', '8877689391');
...
FROM (
		SELECT 
			id,
			totaldistance,
			PERCENT_RANK() OVER (ORDER BY totaldistance DESC) AS percentile_among_peers
		FROM cs2.dailyactivity
		GROUP BY id
	 ) AS sub
WHERE sub.id IN('4319703577', '4558609924','6962181067', '8877689391');
...
FROM (
		SELECT 
			id,
			calories,
			PERCENT_RANK() OVER (ORDER BY calories DESC) AS percentile_among_peers
		FROM cs2.dailyactivity
		GROUP BY id
	 ) AS sub
WHERE sub.id IN('4319703577', '4558609924','6962181067', '8877689391');
...
FROM (
		SELECT 
			...
			PERCENT_RANK() OVER (ORDER BY SUM(veryactiveminutes + fairlyactiveminutes + lightlyactiveminutes) DESC) AS percentile_among_peers
		FROM cs2.dailyactivity
		GROUP BY id
	 ) AS sub
WHERE sub.id IN('4319703577', '4558609924','6962181067', '8877689391');
